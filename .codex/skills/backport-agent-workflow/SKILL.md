...
- `patch_tool.py check`：按 title、`hash title` 或 hash 检查目标分支是否已合入；输出固定为 `title|commit_id|status|git_describe|commit_time|lines_changed`，其中 `lines_changed` 是新增加删除行数。
